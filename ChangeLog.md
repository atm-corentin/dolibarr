...
- NEW : VT-25: On client proposal/order **clone** (devis→devis, commande→commande), recompute every cloned product line's `buy_price_ht` from `product.cost_price` (0 when missing/zero), clear `fk_product_fournisseur_price`, and trace the origin in a hidden line extrafield `clichaumeil_cost_source` (`product_cost_price` / `product_cost_price_zeroed`). Free lines are untouched; no PMP/minimum-supplier/BOM/defineBuyPrice fallback. The devis→commande transformation is explicitly left untouched. Runs in the core clone transaction (a failure rolls back the clone). *22/06/2026* - 1.18.0
- FIX : Make the commission dictionary seeder idempotent at the dictionary level: skip seeding entirely when the entity already holds rows, so module reactivation no longer fails with a duplicate-entry error on the `(entity, role_code, customer_tag)` unique key and never overwrites codes/tags the user has customized through the editable dictionary. *18/06/2026* - 1.17.1
- NEW : R2-ST-6: Propagate the selected supplier proposal buy price (line `subprice` — the supplier-quoted unit price) to the matching client proposal/order line `buy_price_ht` on subcontractor selection (button or supplier proposal acceptance). On draft documents the selling price is recomputed via the Discountrules minimum markup/margin rate (thirdparty > product > global override order); on validated documents only the buy price is updated and the margin is recomputed on read. A persistent link between supplier proposal lines and their client parent line is stored on two new hidden extrafields (`clichaumeil_source_element`, `clichaumeil_source_line_id`) and backfilled on supplier proposal creation; a deterministic fk_product + rang + special_code fallback is used when no link is found and is only applied on unique matches. *16/06/2026* - 1.17.0
- NEW : R1-ACHT-4: Add client RFA management — separate supplier/client tabs on thirdparty card, global client RFA summary list backed by closed customer invoices (STATUS_CLOSED), unified rebuild script and cron job (rfa_type parameter), yearly summary table extended with rfa_type discriminant column. *09/06/2026* - 1.17.0
