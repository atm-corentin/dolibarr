...
- FIX : ACHT-2-ANTALIS: Make the API connection screen understandable for a non-expert admin. Reword the raw API field labels into plain language ("ANTALIS service URL", "ANTALIS customer number (customerID)", "Connection login/password (provided by ANTALIS)"…), add an info-tooltip on every field (what to enter, that the value comes from ANTALIS / what to ask them, an example for the URL, a recommended value for the closure limit), and an intro sentence stating the connection settings are provided by ANTALIS. Lang-only, both fr_FR and en_US. *03/06/2026* - 1.20.2
...
## 1.22
- FIX : ACHT-2-ANTALIS: Show a state placeholder in the ANTALIS API password field ("already set" / "ANTALIS API password") instead of an ambiguous empty field, so an admin can tell a stored password from a missing one. The placeholder is a cue only — the real value is never sent to the browser (still dolEncrypt, never pre-filled). *09/06/2026* - 1.22.2
- FIX : ACHT-2-ANTALIS: Make the price synchronisation idempotent. The supplier unit price (personalUnitPrice / personalPriceQty) is a raw float with more decimals than Dolibarr persists, so comparing it against the rounded stored price always differed and every run re-updated every line (bloating the buy-price history). The incoming price is now rounded to the stored precision (price2num 'MU') before the comparison, the write and the report. *05/06/2026* - 1.22.1
- REFACTOR : ACHT-2-ANTALIS: Merge the ANTALIS connection and behaviour settings into a single FormSetup with one save button (two titled sections), instead of two forms with separate save actions. Removes the updatebehaviour action and the two-FormSetup clobber risk; the page keeps three coherent save scopes (shared settings, ANTALIS configuration, password). *05/06/2026* - 1.22.1
- NEW : ACHT-2-ANTALIS: Make the sync report and the API connections page production-grade. Report: anomalies are surfaced before the per-line changes (they are the reason a notification is sent), the summary becomes a readable multi-line block (plus a one-line compact summary persisted for the admin), price changes show their variation percentage, the run timestamp and duration are recorded, and the mail is sent as a structured HTML body (email-safe inline styles) with a summary table, coloured errors/warnings, products linked to their card (absolute URL, no extra SQL), a subject splitting errors and warnings, and a footer linking back to the configuration. A configurable notification policy (never / errors / errors+warnings / always) and a recipients list are added as settings shared by every API connector. A product-limit test knob runs the sync on the first N products only, to validate on real data in seconds instead of hours. Admin: settings are split into a shared block and a per-connector card (reusable clichaumeilConnectorCardStart/End shell with a configured / to-configure status badge), connection credentials are grouped, test buttons are visually distinguished from save buttons, the last run is shown as coloured badges, and test-mode warnings are merged into a single banner. *05/06/2026* - 1.22.0
- FIX : ACHT-2-ANTALIS: The notification-policy select showed "Never" when unset while the cron actually defaulted to errors+warnings; the select now defaults to the effective policy so the displayed value matches the real behaviour. *05/06/2026* - 1.22.0
