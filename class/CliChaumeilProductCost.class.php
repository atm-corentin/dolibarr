...
class CliChaumeilProductCostCalculator
{
	 ...
	 */
	private const FG_PERCENT_FIELD = 'clichaumeil_fg_percent';

	/**
	 * @var string Extrafield storing the calculated overhead amount.
	 */
	private const FG_AMOUNT_FIELD = 'clichaumeil_pa_fg';
	 ...
	 *
	 * @param Product $product
	 * @return array Array with keys: fg_percent, pa_fg, cost_price (all as numeric values)
	 */
	private static function buildData(Product $product): array
	{
		...
		$fgPercent = self::getExtrafieldValue($product, self::FG_PERCENT_FIELD);

		$paFg = price2num($base * ((float) $fgPercent) / 100, 4);
		$costPrice = price2num($base + (float) $paFg, 4);
	 ...
	 * @return bool
	 */
	private static function valueDiffers($current, $target): bool
	{
		if ($current === null) {
			return true;
		}
	 ...
	 */
	private static function updateProductPriceField(Product $product, string $field, $value, User $user): int
	{
