...
/**
 *  \file       chaumeilrfa_document.php
 *  \ingroup    clichaumeil
 *  \brief      Tab for documents linked to ChaumeilRfa
 */
...
dol_include_once('/clichaumeil/class/chaumeilrfa.class.php');
dol_include_once('/clichaumeil/lib/clichaumeil_chaumeilrfa.lib.php');
...
$langs->loadLangs(array("clichaumeil@clichaumeil", "companies", "other", "mails"));
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->clichaumeil->multidir_output[$object->entity ? $object->entity : $conf->entity]."/chaumeilrfa/".get_exdir(0, 0, 0, 1, $object);
}
...
// Header
// ------
$title = $langs->trans("ChaumeilRfa")." - ".$langs->trans("Files");
//$title = $object->ref." - ".$langs->trans("Files");
$help_url = '';
//Example $help_url='EN:Module_Third_Parties|FR:Module_Tiers|ES:Empresas';
llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-clichaumeil page-card_document');

// Show tabs
$head = chaumeilrfaPrepareHead($object);

print dol_get_fiche_head($head, 'document', $langs->trans("ChaumeilRfa"), -1, $object->picto);
...
$modulepart = 'clichaumeil';
$param = '&id='.$object->id;
//$relativepathwithnofile='chaumeilrfa/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'chaumeilrfa/'.dol_sanitizeFileName($object->ref).'/';
