...
if ($action == 'confirm_delete_ol' && GETPOST('confirm') == 'yes' ) {
    ...
    $comment = new gt_comments($db);
	if($comment->getcountrows(' AND name_module ="product" AND fk_module ='.$id.' AND fk_comment='.$id_delete) > 0){
		$comments =new gt_comments($db);
		$comments->fetchAll('','',0,0,' AND name_module ="product" AND fk_module ='.$id.' AND fk_comment='.$id_delete);
...
$filter = ' AND name_module ="product" AND fk_module ='.$id.' AND (fk_comment IS Null or fk_comment="")';
...
dol_fiche_head($head, 'tab_commentaire', $titre, -1, $picto);
				...
				print '<tr>';
					...
					print '<td>';
						print '<div class="comemnt_parent">';
							...
							if($comments->getcountrows(' AND name_module ="product" AND fk_module ='.$id.' AND fk_comment='.$item->rowid) > 0){
							...
							print $cmt->getcommentschild('product', $id, $item->rowid, 'id', $srch_comment = '', $newcommentfiles, false);
						print '</div>';
					print '</td>';
				print '</tr>';	
			}
