...
$title = $langs->trans('Proposal')." - ".$langs->trans('Comments');
$helpurl = "EN:Customers_Invoices|FR:propals_Clients|ES:Facturas_a_clientes";
...
$filter = ' AND name_module ="propal" AND fk_module ='.$id.' AND (fk_comment IS Null or fk_comment="")';
				...
				print '<tr>';
					...
					print '<td>';
						print '<div class="comemnt_parent">';
							...
							if($comments->getcountrows(' AND name_module ="propal" AND fk_module ='.$id.' AND fk_comment='.$item->rowid) > 0){
							...
							print '<form method="post" action="'.$_SERVER["PHP_SELF"].'" enctype="multipart/form-data">';
								...
								print '<div id="repond_comment_'.$item->rowid.'" class="repond_comment">';
									...
									// print $comments->dolEditorWithSelectUsers($item, 'txtcomment_reponder_', '', $item->rowid, $item->fk_user);
							...
							print $cmt->getcommentschild('propal', $id, $item->rowid, 'id', $srch_comment = '', $newcommentfiles, false);
						print '</div>';
					print '</td>';
				print '</tr>';	
			}
