...
$title = $langs->trans('Shipment')." - ".$langs->trans('Comments');
...
$filter = ' AND name_module ="shipping" AND fk_module ='.$id.' AND (fk_comment IS Null or fk_comment="")';
				...
				print '<tr>';
					...
					print '<td>';
						print '<div class="comemnt_parent">';
							...
							if($comments->getcountrows(' AND name_module ="shipping" AND fk_module ='.$id.' AND fk_comment='.$item->rowid) > 0){
							...
							print $cmt->getcommentschild('shipping', $id, $item->rowid, 'id', $srch_comment = '', $newcommentfiles, false);
						print '</div>';
					print '</td>';
				print '</tr>';	
			}
