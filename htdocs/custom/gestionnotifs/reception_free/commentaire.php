...
$title = $langs->trans('Reception')." - ".$langs->trans('Comments');
...
$filter = ' AND name_module ="reception_free" AND fk_module ='.$id.' AND (fk_comment IS Null or fk_comment="")';
...
$param .= '&id='.$id;
// print_r($object);die();
dol_fiche_head($head, 'tab_commentaire', $langs->trans("Shipment"), -1, $object->picto);
// Shipment card
// Reception card
				...
				print '<tr>';
					...
					print '<td>';
						print '<div class="comemnt_parent">';
							...
							if($comments->getcountrows(' AND name_module ="reception_free" AND fk_module ='.$id.' AND fk_comment='.$item->rowid) > 0){
							...
							print $cmt->getcommentschild('reception_free', $id, $item->rowid, 'id', $srch_comment = '', $newcommentfiles, false);
						print '</div>';
					print '</td>';
				print '</tr>';	
			}
