...
if ($action == 'confirm_delete_ol' && GETPOST('confirm') == 'yes' ) {
	...
	if($comment->getcountrows(' AND name_module ="order_supplier" AND fk_module ='.$id.' AND fk_comment='.$id_delete) > 0){
		$comments =new gt_comments($db);
		$comments->fetchAll('','',0,0,' AND name_module ="order_supplier" AND fk_module ='.$id.' AND fk_comment='.$id_delete);
...
$title = $langs->trans('SupplierOrder')." - ".$langs->trans('Comments');
...
$filter = ' AND name_module ="order_supplier" AND fk_module ='.$id.' AND (fk_comment IS Null or fk_comment="")';
...
$param .= '&id='.$id;
// print_r($object);die();
dol_fiche_head($head, 'tab_commentaire', $langs->trans("Proposal"), -1, $object->picto);
...
if (!empty($conf->projet->enabled)) {
	...
	if ($usercancreate) {
		if ($action != 'classify' && !empty($caneditproject)) {
				...
				print '<tr>';
					...
					print '<td>';
						print '<div class="comemnt_parent">';
							...
							if($comments->getcountrows(' AND name_module ="order_supplier" AND fk_module ='.$id.' AND fk_comment='.$item->rowid) > 0){
							...
							print $cmt->getcommentschild('order_supplier', $id, $item->rowid, 'id', $srch_comment = '', $newcommentfiles, false);
						print '</div>';
					print '</td>';
				print '</tr>';	
			}
		...
		print '<tr>';
			...
			print '<td>';
				print '<form method="post" action="'.$_SERVER["PHP_SELF"].'" enctype="multipart/form-data">';
				    ...
				    print '<input type="hidden" name="id" value="'.$id.'" />';
					print '<input type="hidden" name="page" value="'.$page.'" />';
					print $comments->dolEditorWithSelectUsers($comments, 'txtcomment', $value = '', $row_id = '', $object->user_author_id, $object);
					print $newcommentfiles;
					// print '<textarea class="comment" name="comment" onchange="textarea_autosize(this)" placeholder="'.$langs->trans('your_comment').'"></textarea>';
					print '<input type="submit" value="'.$langs->trans("sauvg").'" class="btnAction">';
				print '</form>';
