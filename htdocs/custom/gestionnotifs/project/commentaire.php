...
if ($action == 'confirm_delete_ol' && GETPOST('confirm') == 'yes' ) {
    ...
    $comment = new gt_comments($db);
	if($comment->getcountrows(' AND name_module ="project" AND fk_module ='.$id.' AND fk_comment='.$id_delete) > 0){
		$comments =new gt_comments($db);
		$comments->fetchAll('','',0,0,' AND name_module ="project" AND fk_module ='.$id.' AND fk_comment='.$id_delete);
...
if (!empty($conf->global->MAIN_HTML_TITLE) && preg_match('/projetnameonly/', $conf->global->MAIN_HTML_TITLE)) $title = $object->ref.(isset($object->thirdparty->name) ? ' - '.$object->thirdparty->name : '').($object->title ? ' - '.$object->title : '');
$help_url = "EN:Module_projets|FR:Module_Projets|ES:M&oacute;dulo_Proyectos";
...
$filter = ' AND name_module ="project" AND fk_module ='.$id.' AND (fk_comment IS Null or fk_comment="")';
...
dol_fiche_head($head, 'tab_commentaire', $langs->trans("projet"), -1, ($object->public ? 'projetpub' : 'project'));
				...
				print '<tr>';
					...
					print '<td>';
						print '<div class="comemnt_parent">';
							...
							if($comments->getcountrows(' AND name_module ="project" AND fk_module ='.$id.' AND fk_comment='.$item->rowid) > 0){
							...
							print $cmt->getcommentschild('project', $id, $item->rowid, 'id', $srch_comment = '', $newcommentfiles, false);
						print '</div>';
					print '</td>';
				print '</tr>';	
			}
