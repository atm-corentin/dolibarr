...
if ($action == 'confirm_delete_ol' && GETPOST('confirm') == 'yes' ) {
	...
	if($comment->getcountrows(' AND name_module ="invoice_supplier" AND fk_module ='.$id.' AND fk_comment='.$id_delete) > 0){
		$comments =new gt_comments($db);
		$comments->fetchAll('','',0,0,' AND name_module ="invoice_supplier" AND fk_module ='.$id.' AND fk_comment='.$id_delete);
...
$filter = ' AND name_module ="invoice_supplier" AND fk_module ='.$id.' AND (fk_comment IS Null or fk_comment="")';
...
$param .= '&facid='.$id;
// print_r($object);die();
dol_fiche_head($head, 'tab_commentaire', $langs->trans("SupplierInvoice"), -1, 'supplier_invoice');
				...
				print '<tr>';
					...
					print '<td>';
						print '<div class="comemnt_parent">';
							...
							if($comments->getcountrows(' AND name_module ="invoice_supplier" AND fk_module ='.$id.' AND fk_comment='.$item->rowid) > 0){
							...
							print $cmt->getcommentschild('invoice_supplier', $id, $item->rowid, 'id', $srch_comment = '', $newcommentfiles, false);
						print '</div>';
					print '</td>';
				print '</tr>';	
			}
		...
		print '<tr>';
			...
			print '<td>';
				print '<form method="post" action="'.$_SERVER["PHP_SELF"].'" enctype="multipart/form-data">';
				    ...
				    print '<input type="hidden" name="id" value="'.$id.'" />';
					print '<input type="hidden" name="page" value="'.$page.'" />';
					$author = ((floatval(DOL_VERSION) < 19) ? $object->fk_user_author : $object->author);
					print $comments->dolEditorWithSelectUsers($comments, 'txtcomment', $value = '', $row_id = '', $author, $object);
					print $newcommentfiles;
					// print '<textarea class="comment" name="comment" onchange="textarea_autosize(this)" placeholder="'.$langs->trans('your_comment').'"></textarea>';
					print '<input type="submit" value="'.$langs->trans("sauvg").'" class="btnAction">';
				print '</form>';
