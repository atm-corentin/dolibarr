...
$id = (GETPOST('id', 'int') ? GETPOST('id', 'int') : GETPOST('id', 'int'));
$object->fetch($id);
...
if($action == 'create'){
	...
	if(!empty($comment)){
		$data = [
			'fk_user' => $user->id,
			'fk_module' => $id,
			'action_socid' => intval($object->socid),
...
if ($action == 'confirm_delete_ol' && GETPOST('confirm') == 'yes' ) {
    ...
    $comment = new gt_comments($db);
	if($comment->getcountrows(' AND name_module ="action" AND fk_module ='.$id.' AND fk_comment='.$id_delete) > 0){
		$comments =new gt_comments($db);
		$comments->fetchAll('','',0,0,' AND name_module ="action" AND fk_module ='.$id.' AND fk_comment='.$id_delete);
...
llxHeader(array(), $title.' - '.$modname,'','','','',$morejs,0,0);
...
if(!empty($conf->global->GESTIONNOTIFS_LIER_COMMENT_EVENET_COMMENT_THIRDPARTY)){
	$filter = ' AND (name_module ="action" AND fk_module ='.$id.' AND (fk_comment IS Null or fk_comment="")) OR (name_module ="societe" AND fk_module ='.intval($object->socid).' AND (fk_comment IS Null or fk_comment=""))';
}else{
	$filter = ' AND name_module ="action" AND fk_module ='.$id.' AND (fk_comment IS Null or fk_comment="")';
}
...
dol_fiche_head($head, 'tab_commentaire', $langs->trans("Events"), -1, 'event');
					...
					print '<tr>';
						...
						print '<td>';
							print '<div class="comemnt_parent">';
								...
								if($comments->getcountrows(' AND name_module ="action" AND fk_module ='.$id.' AND fk_comment='.$item->rowid) > 0){
						...
						// $filter = ' OR (name_module ="societe" AND fk_module ='.$object->socid.' AND fk_comment IS NOT NULL)';

								print $cmt->getcommentschild('action', $id, $item->rowid, 'id', $srch_comment = '', $newcommentfiles, false);
							print '</div>';
						print '</td>';
					print '</tr>';	
				}
