...
if($action == 'create'){
	...
	if(!empty($comment)){
		...
		if($result){
			...
			header('Location:commentaire.php?rowid='.$id);
		}
	}
}
if($action == 'reponder'){
...
if ($action == 'confirm_delete_ol' && GETPOST('confirm') == 'yes' ) {
	...
	if($comment->getcountrows(' AND name_module ="member" AND fk_module ='.$id.' AND fk_comment='.$id_delete) > 0){
		$comments =new gt_comments($db);
		$comments->fetchAll('','',0,0,' AND name_module ="member" AND fk_module ='.$id.' AND fk_comment='.$id_delete);
    ...
    if ($error == 1) {
        ...
        header('Location: commentaire.php?rowid='.$id.'&page='.$page);
        exit;
    }
    else {      
...
if($action == "delete"){
	$id_delete = GETPOST('id_delete');
    print $form->formconfirm("commentaire.php?rowid=".$id."&id_delete=".$id_delete."&page=".$page,$langs->trans('Confirmation') , $langs->trans('ConfirmDeleteObject'),"confirm_delete", 'commentaire.php?page='.$page, 0, 1);
}
...
$filter = ' AND name_module ="member" AND fk_module ='.$id.' AND (fk_comment IS Null or fk_comment="")';
...
dol_fiche_head($head, 'tab_commentaire', $langs->trans("member"), -1, $object->picto);
					...
					print '<td>';
						print '<div class="comemnt_parent">';
							...
							if($user->id == $item->fk_user){
								...
								print '</a>';
								print '<a href="commentaire.php?rowid='.$id.'&id_delete='.$item->rowid.'&action=delete" class="removecomment">'.img_delete($langs->trans('Delete')).'</a>';
							}
							print '<a class="repond" id="repond_'.$item->rowid.'" data-id="'.$item->rowid.'">'.$langs->trans("repond").'</a>';
							if($comments->getcountrows(' AND name_module ="member" AND fk_module ='.$id.' AND fk_comment='.$item->rowid) > 0){
							...
							print '<form method="post" action="'.$_SERVER["PHP_SELF"].'" enctype="multipart/form-data">';
								...
								print '<div id="edit_comment_'.$item->rowid.'" class="edit_comment">';
									...
									print $filesdiv;
									// print '<textarea class="comment" name="comment" placeholder="'.$langs->trans('your_comment').'">'.dol_string_onlythesehtmltags($item->comment).'</textarea>';
							...
							print '<form method="post" action="'.$_SERVER["PHP_SELF"].'" enctype="multipart/form-data">';
								...
								print '<div id="repond_comment_'.$item->rowid.'" class="repond_comment">';
									...
									// print '<textarea class="comment" name="comment" placeholder="'.$langs->trans('your_comment').'"></textarea>';
							...
							print $cmt->getcommentschild('member', $id, $item->rowid, 'id', $srch_comment = '', $newcommentfiles, false);
						print '</div>';
					print '</td>';
				print '</tr>';	
			}
