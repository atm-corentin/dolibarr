...
if ($action == 'confirm_delete_ol' && GETPOST('confirm') == 'yes' ) {
	...
	if($comment->getcountrows(' AND name_module ="contact" AND fk_module ='.$id.' AND fk_comment='.$id_delete) > 0){
		$comments =new gt_comments($db);
		$comments->fetchAll('','',0,0,' AND name_module ="contact" AND fk_module ='.$id.' AND fk_comment='.$id_delete);
...
$filter = ' AND name_module ="contact" AND fk_module ='.$id.' AND (fk_comment IS Null or fk_comment="")';
...
dol_fiche_head($head, 'tab_commentaire', $langs->trans("Contact"), -1, (isset($object->public) ? 'contactpub' : 'contact'));
				...
				print '<tr>';
					...
					print '<td>';
						print '<div class="comemnt_parent">';
							...
							if($comments->getcountrows(' AND name_module ="contact" AND fk_module ='.$id.' AND fk_comment='.$item->rowid) > 0){
							...
							print $cmt->getcommentschild('contact', $id, $item->rowid, 'id', $srch_comment = '', $newcommentfiles, false);
						print '</div>';
					print '</td>';
				print '</tr>';	
			}
