...
$title = $langs->trans('Order')." - ".$langs->trans('Comments');
...
$filter = ' AND name_module ="commande" AND fk_module ='.$id.' AND (fk_comment IS Null or fk_comment="")';
...
$param .= '&id='.$id;
// print_r($object);die();
dol_fiche_head($head, 'tab_commentaire', $langs->trans("Order"), -1, $object->picto);
				...
				print '<tr>';
					...
					print '<td>';
						print '<div class="comemnt_parent">';
							...
							if($comments->getcountrows(' AND name_module ="commande" AND fk_module ='.$id.' AND fk_comment='.$item->rowid) > 0){
							...
							print $cmt->getcommentschild('commande', $id, $item->rowid, 'id', $srch_comment = '', $newcommentfiles, false);
						print '</div>';
					print '</td>';
				print '</tr>';	
			}
