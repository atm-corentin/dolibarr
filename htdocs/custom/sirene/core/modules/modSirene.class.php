...
/**
 *    \defgroup   sirene     Module Sirene
 *  \brief      Example of a module descriptor.
 *                Such a file must be copied into htdocs/sirene/core/modules directory.
 *  \file       htdocs/sirene/core/modules/modSirene.class.php
 *  \ingroup    sirene
 *  \brief      Description and activation file for module Sirene
 */
...
/**
 *  Description and activation class for module Sirene
 */
class modSirene extends DolibarrModules
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->numero = 163027;        // TODO Go on page http://wiki.dolibarr.org/index.php/List_of_modules_id to reserve id number for your module
		// Key text used to identify module (for permissions, menus, etc...)
		$this->rights_class = 'sirene';
		...
		$this->module_parts = array(
			'triggers'		=> 1,
			'dictionaries'	=> 1,
			'hooks'			=> array('thirdpartycard', 'idprofurl'),
		);
		...
		$this->config_page_url = array("setup.php@sirene");

		// Dependencies
		$this->hidden = false;            // A condition to hide module
		$this->depends = array('modSociete',
							   'modAdvanceDictionaries');        // List of modules id that must be enabled if this module is enabled
		$this->requiredby = array();    // List of modules id to disable if this one is disabled
		$this->conflictwith = array('modCodeNaf');    // List of modules id this module is in conflict with

		$this->langfiles = array("sirene@sirene", "opendsi@sirene");
		...
		// );
		$this->const = array(
			0 => array('CODENAF_CSV_SEPARATOR_TO_USE', 'chaine', ';', '', 0, 'current', 0),
			1 => array('CODENAF_CSV_ENCLOSURE_TO_USE', 'chaine', '"', '', 0, 'current', 0),
			2 => array('CODENAF_CSV_ESCAPE_TO_USE', 'chaine', '\\', '', 0, 'current', 0),
			3 => array('SIRENE_VERIFICATION_SIRET_URL', 'chaine', 'https://annuaire-entreprises.data.gouv.fr/entreprise/', '', 0, 'current', 0),
			4 => array('MODULE_SIRENE_VERSION', 'chaine', $this->version, '', 0, 'allentities', 0),
		);
		...
		if (!isset($conf->sirene) || !isset($conf->sirene->enabled)) {
			$conf->sirene = new stdClass();
			$conf->sirene->enabled = 0;
		}
		...
		*/
		...
		// Cronjobs
		$this->cronjobs = array(
			0 => array('label'         => $langs->trans("SireneCronCheckCompanies"), 'jobtype' => 'method',
					   'class'         => '/sirene/class/sirene.class.php', 'objectname' => 'Sirene',
					   'method'        => 'cronSirene', 'parameters' => '', 'comment' => '', 'frequency' => 10,
					   'unitfrequency' => 60, 'test' => '$conf->sirene->enabled', 'status' => 0),
	...
	public function init($options = '')
	{
		...
		$langs->loadLangs(array('sirene@sirene', 'opendsi@sirene'));

		$sql = array();

		$this->_load_tables('/sirene/sql/patch/');

		// Create tables of all dictionaries
		$version_min = '';
		if (dol_include_once('/advancedictionaries/class/dictionary.class.php')) {
			if (version_compare(getSireneDolGlobalString('ADVANCEDICTIONARIES_VERSION'), $version_min) >= 0) {
				$dictionaries = Dictionary::fetchAllDictionaries($this->db, 'sirene');
				...
				setEventMessage($langs->trans('SireneErrorModuleWrongVersion', "AdvanceDictionaries", $version_min) . ' ' . $langs->trans('SireneErrorModuleNotFoundDownloadHere', 'https://git.open-dsi.fr/dolibarr-extension/advancedictionaries'), 'errors');
				return 0;
			}
		} else {
			setEventMessage($langs->trans('SireneErrorModuleNotFound', "AdvanceDictionaries") . ' ' . $langs->trans('SireneErrorModuleNotFoundDownloadHere', 'https://git.open-dsi.fr/dolibarr-extension/advancedictionaries'), 'errors');
			return 0;
		}

		$this->_load_tables('/sirene/sql/');
		...
		//$result = $extrafields->update('sirene_cron_date', $langs->trans("SireneCronDate"), 'datetime', '', 'societe', 0, 0, 103, null, 0, '', $visibilityListOnly, '', '', '', '', 'sirene@sirene', '$conf->sirene->enabled');

		// v10.3.0
		$result = $extrafields->delete('sirene_status', 'societe');
		$result = $extrafields->delete('sirene_cron_date', 'societe');
		...
		if ($result < 0) {
			setEventMessage('Error import code NAF csv: ' . $codenaf->errorsToString(), 'errors');
			return 0;
		} else {
