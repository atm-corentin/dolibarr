...
//                        if (!$this->db->num_rows($resql)) {
					...
					$sql = 'ALTER TABLE ' . MAIN_DB_PREFIX . $table_name .
						' ADD CONSTRAINT fk_' . $initial . '_cbl_' . $field['name'] . '_b FOREIGN KEY (' . $cq . $this->getDestinationColumnAssociationTableName($field) . $cq . ') REFERENCES ' .
						MAIN_DB_PREFIX . $InfoFieldList[0] . ' (' . $cq . $keyList . $cq . ');';
...
//                    }
	 ...
	 */
	static public function fetchAllDictionaries($db, $module = '', $family = '', $root_path = '')
	{
		global $conf;
	 ...
	 */
	static public function getDictionary($db, $module = '', $name = '', $old_id = 0, $root_path = '')
	{
		$dictionary = null;
	 ...
	 */
	static public function getDictionaryLine($db, $module = '', $name = '', $old_id = 0, $root_path = '')
	{
	 ...
	 * @return  void
	 */
	static public function getDictionaryModuleAndNameFromID($old_id, &$module, &$name)
	{
		$module = '';
		$name = '';
...
class DictionaryLine extends CommonObjectLine
{
	...
	{
		//parent::__construct($db); not exist on v10
		$this->db = $db;
	...
	public function update($fieldsValue, $user, $noTrigger = 0)
	{
		...
		if ($this->checkFieldsValues($fieldsValue) > 0) {
			...
			if ($this->dictionary->is_multi_entity && $this->dictionary->has_entity) {
				$sql .= ' AND ' . $cq . $this->dictionary->entity_field . $cq . ' = ' . $this->oldcopy->entity;
			}
			...
			if (!$error) {
				...
				$this->fields = $this->oldcopy->fields;
				$this->db->rollback();
				return -1;
			}
		} else {
	...
	*/
	public function active($status, $user, $noTrigger = 0)
	{
		...
		if (!$error) {
			...
			$this->active = $this->oldcopy->active;
			$this->db->rollback();
			return -1;
		}
	}
	...
	public function setEntity($user, $entity, $noTrigger = 0)
	{
		...
		if (!$error) {
			...
			$this->entity = $this->oldcopy->entity;
			$this->db->rollback();
			return -1;
		}
	}
