	...
	while ($i < min($num, $limit)) {
		...
		if(!empty($obj->holiday)){
			...
			if($holidays){
				foreach ($holidays as $key => $idsholiday) {
					...
					$halfday = !empty($arrholiday[4]) ? $arrholiday[4] : 0;
					...
					$nbDays = num_open_day($h_start, $h_end, 0, 1, 0);
					if($nbDays>0){
		...
		if($timespasse){
			foreach ($timespasse as $key => $linetime) {
				...
				if(!empty($arrtime[0])){
					...
					$daysconsombyuser[$arrtime[1]][] = array('date'=>$arrtime[3], 'time'=>$arrtime[2], 'task'=>$ganttid);

					// $baseline = array();
					// // { "id": 4, "task_id": "task26", "start_date": "2025-07-06 00:00:00", "duration": 1, "end_date": "2025-07-10 00:00:00" },
					// $indexbaseline++;
					// $start = $db->jdate($arrtime[3]);
					// $duree = $arrtime[2];
					// $end = $start+$duree;

					// $baseline['id'] = $indexbaseline;
					// $baseline['task_id'] = $data['id'];
					// $baseline['start_date'] = dol_print_date($start, "%Y-%m-%d %H:%M", 'tzserver');
					// $baseline['end_date'] = dol_print_date($end, "%Y-%m-%d %H:%M", 'tzserver');
					// // $baseline['duration'] = $duree/(3600);

					// $_baselines	.= json_encode($baseline).',';
		...
		if($obj->typetable == 'task' && $viewmode == 'byresource' && $ganttproadvanced->byressource_type_contacts == 'per_principal_contributeur') {
			...
			foreach ($contactscontributors as $tmpkey => $contact_user_id) {
				...
				if(!empty($daysconsombyuser[$contact_user_id]) && empty($all_baselines[$origindata['id']])){
				
					foreach ($daysconsombyuser[$contact_user_id] as $key => $value) {
						// $baseline = array();

						$baseline = array();
						$indexbaseline++;

						$start = $db->jdate($value['date']);
						$duree = $value['time'];
						$end = $start+$duree;

						$baseline['id'] = $indexbaseline;
						$baseline['task_id'] = $origindata['id'];
						$baseline['start_date'] = dol_print_date($start, "%Y-%m-%d %H:%M", 'tzserver');
						$baseline['end_date'] = dol_print_date($end, "%Y-%m-%d %H:%M", 'tzserver');

						// $daystart = dol_mktime(0, 0, 0, date('m', $start), date('d', $start), date('Y', $start));
						// $dayend = dol_mktime(23, 59, 0, date('m', $end), date('d', $end), date('Y', $end));

						// $baseline['start_date'] = dol_print_date($daystart, "%Y-%m-%d %H:%M", 'tzserver');
						// $baseline['end_date'] = dol_print_date($dayend, "%Y-%m-%d %H:%M", 'tzserver');

						$baseline['duration'] = $value['time']/(3600);
						
						$_baselines	.= json_encode($baseline).',';
						$all_baselines[$origindata['id']]=1;
					}
				}
		...
		if($obj->typetable == 'task' && $viewmode == 'byresource') {
			...
			if(!empty($dataholiday[$isuseraffected])){
				...
				$d_end = strtotime('+ '.(float)$nbdaysconge.' day ', $_end);
			...
			if(!empty($daysconsombyuser[$isuseraffected])){
				foreach ($daysconsombyuser[$isuseraffected] as $key => $value) {
					$baseline = array();
					// { "id": 4, "task_id": "task26", "start_date": "2025-07-06 00:00:00", "duration": 1, "end_date": "2025-07-10 00:00:00" },
					$indexbaseline++;
					$baseline['id'] = $indexbaseline;
...
$_baslines = '{ "id": 1, "task_id": "task24", "start_date": "2025-06-15 00:00:00", "duration": 1, "end_date": "2025-06-16 00:00:00" },
	{ "id": 2, "task_id": "task24", "start_date": "2025-06-12 00:00:00", "duration": 3, "end_date": "2025-06-14 00:00:00" },
	{ "id": 3, "task_id": "task24", "start_date": "2025-06-17 00:00:00", "duration": 2, "end_date": "2025-06-18 00:00:00" },

	{ "id": 4, "task_id": "task26", "start_date": "2025-07-06 00:00:00", "duration": 1, "end_date": "2025-07-10 00:00:00" },
	{ "id": 5, "task_id": "task26", "start_date": "2025-07-11 00:00:00", "duration": 1, "end_date": "2025-07-12 00:00:00" },
	{ "id": 6, "task_id": "task26", "start_date": "2025-07-13 00:00:00", "duration": 1, "end_date": "2025-07-14 00:00:00" },
	{ "id": 7, "task_id": "task26", "start_date": "2025-07-15 00:00:00", "duration": 3, "end_date": "2025-07-18 00:00:00" },
	{ "id": 8, "task_id": "task26", "start_date": "2025-07-19 00:00:00", "duration": 1, "end_date": "2025-07-20 00:00:00" },

	{ "id": 9, "task_id": "task27", "start_date": "2025-07-10 00:00:00", "duration": 3, "end_date": "2025-07-13 00:00:00" },
	{ "id": 10, "task_id": "task27", "start_date": "2025-07-14 00:00:00", "duration": 2, "end_date": "2025-07-16 00:00:00" },
	{ "id": 11, "task_id": "task27", "start_date": "2025-07-16 00:00:00", "duration": 1, "end_date": "2025-07-17 00:00:00" },
	{ "id": 13, "task_id": "task27", "start_date": "2025-07-03 00:00:00", "duration": 1, "end_date": "2025-07-04 00:00:00" },
	{ "id": 14, "task_id": "task27", "start_date": "2025-07-05 00:00:00", "duration": 1, "end_date": "2025-07-06 00:00:00" },
	{ "id": 22, "task_id": "task25", "start_date": "2025-07-11 00:00:00", "duration": 1, "end_date": "2025-07-12 00:00:00" },
	{ "id": 23, "task_id": "task25", "start_date": "2025-07-12 00:00:00", "duration": 1, "end_date": "2025-07-13 00:00:00" },
	{ "id": 24, "task_id": "task25", "start_date": "2025-07-13 00:00:00", "duration": 1, "end_date": "2025-07-14 00:00:00" },
	{ "id": 25, "task_id": "task25", "start_date": "2025-07-14 00:00:00", "duration": 1, "end_date": "2025-07-15 00:00:00" },
	{ "id": 26, "task_id": "task25", "start_date": "2025-07-15 00:00:00", "duration": 1, "end_date": "2025-07-16 00:00:00" },
	{ "id": 27, "task_id": "task25", "start_date": "2025-07-16 00:00:00", "duration": 1, "end_date": "2025-07-17 00:00:00" },
	{ "id": 28, "task_id": "task25", "start_date": "2025-07-17 00:00:00", "duration": 1, "end_date": "2025-07-18 00:00:00" },
	{ "id": 29, "task_id": "task25", "start_date": "2025-07-18 00:00:00", "duration": 1, "end_date": "2025-07-19 00:00:00" },
	{ "id": 30, "task_id": "task25", "start_date": "2025-07-19 00:00:00", "duration": 1, "end_date": "2025-07-20 00:00:00" },
	{ "id": 31, "task_id": "task25", "start_date": "2025-07-20 00:00:00", "duration": 1, "end_date": "2025-07-21 00:00:00" },
	{ "id": 32, "task_id": "task25", "start_date": "2025-07-21 00:00:00", "duration": 1, "end_date": "2025-07-22 00:00:00" },
	{ "id": 33, "task_id": "task25", "start_date": "2025-07-22 00:00:00", "duration": 1, "end_date": "2025-07-23 00:00:00" },
	{ "id": 34, "task_id": "task27", "start_date": "2025-07-08 00:00:00", "duration": 1, "end_date": "2025-07-09 00:00:00" }';
...
// echo '<link rel="stylesheet" href="https://docs.dhtmlx.com/gantt/codebase/dhtmlxgantt.css?v=9.0.13">';
// echo '<link rel="stylesheet" href="https://docs.dhtmlx.com/gantt/samples/common/controls_styles.css?v=9.0.13">';
...
print	'<style>
	...
	.resources_cell .gantt_task .gantt_task_scale .gantt_scale_cell {
		font-size: 8px
	}
	

	.gantt_resource_marker_ok {
	    background: #1BC297 !important;
	}
	
	.gantt_resource_marker_overtime {
	    background: #E3334E !important;
	}
	.gantt_task_baseline{background:#bc35d5 !important; display: none;}
	...
	.gantt_task_baseline{position:absolute;opacity:.9;background:#ffd180;z-index:0;overflow:hidden}
	.gantt_task_baseline.gantt_milestone_baseline{transform:rotate(45deg)}
	.gantt_section_baselines{overflow-y:auto;display:flex;flex-direction:column;gap:4px}
	.gantt_custom_button.gantt_add_baselines{margin-left:0}
	.gantt_cal_cover_test .gantt_cal_light {display: block !important;}
	.gantt_cal_light_wide .gantt_cal_larea {height: 80vh;}
	.dhx_cal_ltitle_descr, .gantt_cal_ltitle_controls{display: inline-block;}

	.group_row, .group_row.odd, .gantt_task_row.group_row {background-color: rgba(232, 232, 232, 0.6) !important;}
	.ganttproadvancedformindex:has(input[name="scale"][value="hour"]:checked) .baseline-layer .custom-baseline{display: none}
	.ganttproadvancedformindex:has(input[name="scale"][value="hour"]:checked) .gantt_task_baseline{ display: block;}
</style>';
...
if((isset($conf->trellotasks) && $conf->trellotasks->enabled) || $trellotasksplus_enabled || $ganttproadvanced->viewingtasksbyresources){
	$head = GanttKanbanAdminPrepareHead($tosendinurl);
	dol_fiche_head($head, 'livehours', '', -1,  '');
}
...
echo '<script src="js/dhtmlxgantt_v2.js?v='.$mve.'"></script>';
// echo '<script src="https://docs.dhtmlx.com/gantt/codebase/dhtmlxgantt.js?v=9.0.14"></script>';
...
require 'ganttproadvanced_script-functions.php';
require 'ganttproadvanced_script_resource.php';
