...
if ($resql) {
	...
	while ($i < min($num, $limit)) {
		...
		if($obj->typetable == 'project') {
			...
			if(empty($data['start_date']) && !empty($datesproj[$ganttid]['start']) && !empty(min($datesproj[$ganttid]['start']))){
				$data['start_date'] = dol_print_date(min($datesproj[$ganttid]['start']), "%Y-%m-%d %H:%M", 'tzserver');
			}
			if(empty($data['end_date']) && !empty($datesproj[$ganttid]['end']) && !empty(min($datesproj[$ganttid]['end']))){
