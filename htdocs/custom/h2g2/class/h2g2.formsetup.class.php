 ...
 * Copyright (C) 2024-2025  Ravi TREBUCHET    <ravi@code42.fr>
 *
...
class H2G2FormSetup
{
	 ...
	 */
	public function __construct($db, $moduleName = 'h2g2', $outputLangs = false)
	{
		global $langs;
		$this->db = $db;
		...
		if ($outputLangs) $this->langs = $outputLangs;
	...
	/**
	 * Set target module to the form (used to store files in the right module)
	 *
	 * @param string $moduleName Module name
	 * @return void
	 */
	 ...
	 * @param  bool    $first       If this is the first form we set this to true
	 * @param  bool    $last        If this is the last form we set this to true
	 * @return    string                html output
	 */
	public function generateOutput($editMode = false, $first = true, $last = true)
	{
		...
		if ($reshook > 0) return $hookmanager->resPrint;
			...
			if ($editMode && $first) {
				$out .= '<form ' . self::generateAttributesStringFromArray($this->formAttributes) . ' enctype="multipart/form-data">';
				$out .= '<input id="form-name" type="hidden" name="form-name" value="' . GETPOST("form-name") . '">';
	...
	public function generateTableOutput($editMode = false)
	{
		...
		if ($reshook > 0) return $hookmanager->resPrint;
			...
			$out .= '<h2 data-title="' . $this->title . '" data-icon="' . $this->icon . '" data-icon-style="' . $this->iconStyle . '">' . $this->langs->trans($this->title) . '</h2>';
	 ...
	 * generateLineOutput
	 *
	 * @param  H2G2FormSetupItem    $item        the setup item
	...
	public function generateLineOutput($item, $editMode = false)
	{
		...
		if ($item->enabled == 1) {
			...
			$fullSize = false;
			if ($item->isFullSize()) $fullSize = true;

			$this->setupNotEmpty++;
			$out .= '<tr class="H2G2-conf-line ' . $trClass . '">';

			$out .= '<td class="col-setup-title"';
			if ($fullSize) $out .= ' colspan="2"';
			$out .= '>';
			$out .= '<span id="helplink' . $item->confKey . '" class="spanforparamtooltip">';
			if ($fullSize) $out .= '<span class="fa fa-level-up-alt" style="transform: rotate(180deg);"></span> ';
			$tooltipType = "";
			if ($item->getType() == "html") $tooltipType = "tooltip";
			$out .= $this->form->textwithpicto($item->getNameText(), $item->getHelpText(), 1, 'info', 'h2g2-pointer', 0, 3, $tooltipType);
			$out .= '</span>';
			if ($editMode && $item->isAutoSave()) $out .= $item->generateSaveButton();

			$out .= '</td>';

			if ($fullSize) $out .= '</tr><tr>';
			$out .= '<td class="H2G2-value';
			$out .= ' H2G2-' . $item->getType();
			$out .= '" ' . ($fullSize ? 'colspan="2"' : '') . '>';

			if ($editMode) $out .= $item->generateInputField();
			else $out .= $item->generateOutputField();
	...
	public function addItemFromParams($confKey, $params)
	{
		...
		$item = new H2G2FormSetupItem($confKey);
	 ...
	 * @param  string    $usedTable            Table used to get constant value (otherwise, it will check on llx_const table)
	 ...
	 * @return H2G2FormSetupItem the new setup item created
	 */
	public function newItem($confKey, $usedTable = "", $targetItemKey = false, $insertAfterTarget = false)
	{
		global $langs;

		$item = new H2G2FormSetupItem($confKey, $this->moduleName, $usedTable);
		...
		$help = $item->confKey . "Help";
		if ($langs->trans($help) != $help) $item->helpText = $langs->trans($help);
	 ...
	 *
	 * @param  H2G2FormSetupItem    $firstItem    formSetup item
	 * @param  H2G2FormSetupItem    $secondItem    formSetup item
	 * @return    int                    Return compare result
	 */
	public function itemSort(H2G2FormSetupItem $firstItem, H2G2FormSetupItem $secondItem)
	{
		if (empty($firstItem->rank)) {
			$firstItem->rank = 0;
		}
		if (empty($secondItem->rank)) {
			$secondItem->rank = 0;
		}
		if ($firstItem->rank == $secondItem->rank) {
			return 0;
		}
		return ($firstItem->rank < $secondItem->rank) ? -1 : 1;
	}
	 ...
	 * @param  string    $iconStyle     If you want to customize icon
	 * @return self
	 */
	public function setTitle($title, $icon, $iconStyle = "")
	{
		$this->title = $title;
		$this->icon = $icon;
...
/**
 * This class help to create item for class H2G2FormSetup
 */
class H2G2FormSetupItem
{
	/**
	 * @var DoliDB Database handler.
	 */
	...
	/** @var string Table used for constant registration */
	public $usedTable = "";
	...
	/**
	 * If this field is HTML type, we can set it to full size, it will take all the available width
	 *
	 * @var bool $fullSize
	 */
	...
	/**
	 * If autoSave is set to true, we have a new button to save dynamically this field
	 *
	 * @var bool $autoSave
	 */
	 ...
	 * @param  string    $confKey    the conf key used in database
	 * @param  string    $moduleName    Module name
	 * @param  string    $usedTable    the conf key used in database
	 */
	public function __construct($confKey, $moduleName = 'h2g2', $usedTable = "")
	{
		...
		// instantiate module name variable
		$this->moduleName = $moduleName;

		// Construct help text if translation key exists
		if ($this->langs->trans($this->confKey . 'Tooltip') != $this->confKey . 'Tooltip') $this->helpText = $this->langs->trans($this->confKey . 'Tooltip');

		$this->confKey = $confKey;
		if (!empty($usedTable)) $this->usedTable = $usedTable;
	...
	{
		$this->fieldValue = getExternalConst($this->confKey, ($this->usedTable ?? ""));
	...
	{
		global $hookmanager, $conf, $db, $langs;

		$moduleName = $this->moduleName;
		if (empty($this->moduleName)) $moduleName = 'h2g2';
		...
		if ($this->type == 'image') {
			if (dol_mkdir($conf->$moduleName->dir_output . '/uploads/') >= 0) {
				$dirforimage = $conf->$moduleName->dir_output . '/uploads/';
				if (isset($_FILES[$this->confKey]["tmp_name"])) {
					$extension = pathinfo($_FILES[$this->confKey]['name'], PATHINFO_EXTENSION);
					$dest_file = $this->confKey . '.' . $extension;
					if (isset($this->fieldValue) && $this->fieldValue && file_exists($conf->$moduleName->dir_output . '/uploads/' . $this->fieldValue)) {
						dol_delete_file($conf->$moduleName->dir_output . '/uploads/' . $this->fieldValue);
					}
					$moved = dol_move_uploaded_file($_FILES[$this->confKey]['tmp_name'], $dirforimage . $dest_file, 1, 0, $_FILES[$this->confKey]['error']);
					if ($moved > 0) {
						setExternalConst($db, $this->confKey, $dest_file, 'string', 1, '', $conf->entity, $this->usedTable);
						return 1;
					} else return -1;
				}
			} else {
				setEventMessages('H2G2DirectoryNotAccessible', null, 'errors');    // API documented error
			}
		} else if ($this->type != 'title') {
			$result = setExternalConst($this->db, $this->confKey, $this->fieldValue, 'chaine', 0, '', $this->entity, $this->usedTable);
			if ($result < 0) {
				return -1;
			} else {
				return 1;
		...
		if ($this->type != 'title') {
			if (preg_match('/category:/', $this->type)) {
				if (GETPOST($this->confKey, 'int') == '-1') {
					...
					$constValue = GETPOST($this->confKey, 'int');
				}
			} elseif ($this->type == 'multiselect') {
				$val = GETPOST($this->confKey, 'array');
				if ($val && is_array($val)) {
					$constValue = implode(',', $val);
				}
			} elseif ($this->type == 'html') {
				$constValue = GETPOST($this->confKey, 'restricthtml');
			} else {
				$constValue = GETPOST($this->confKey, 'alpha');
			}

			if (isset($constValue)) $this->fieldValue = $constValue;
	...
	public function generateInputField()
	{
		...
		if ($this->type == 'title') {
			...
			// Print file form and upload button
			$out .= '<input type="file" name="' . $this->confKey . '" accept="image/*">';
			// Try to print image if it is found
			$image = ($conf->$moduleName->dir_output ?? "") . '/uploads/' . ($this->fieldValue ?? '');
			if (isset($this->confKey) && $this->confKey != '' && file_exists($image)) {
				$dimensions = dol_getImageSize($image);
				if (!empty($dimensions['width']) && $dimensions['width'] > 200) {
					$dimensions['height'] = ($dimensions['height'] * 200) / $dimensions['width'];
					$dimensions['width'] = 200;
				}
				if (!empty($dimensions['height']) && $dimensions['height'] > 100) {
					$dimensions['width'] = ($dimensions['width'] * 100) / $dimensions['height'];
					$dimensions['height'] = 100;
				}
				$out .= '<img align="right" height="' . ($dimensions['height'] ?? 0) . '" width="' . ($dimensions['width'] ?? 0) . '" src="' . DOL_URL_ROOT . '/viewimage.php?modulepart=' . $moduleName . '&file=uploads/' . ($this->fieldValue ?? '') . '">';
			} else {
				$out .= '<img align="right" height="40" src="' . DOL_URL_ROOT . '/public/theme/common/nophoto.png">';
			}
		} else {
	...
	public function generateInputFieldText()
	{
		...
		return '<input ' . H2G2FormSetup::generateAttributesStringFromArray($this->fieldAttr) . ' />';
	}
	...
	public function generateInputFieldHtml()
	{
		...
		$doleditor = new DolEditor($this->confKey, $this->fieldValue, '', (!empty($this->height) ? $this->height : 160), 'dolibarr_notes', '', false, false, $conf->fckeditor->enabled, ROWS_5, '90%');
	...
	/**
	 * Generate input field for multi select (you can select more that one item)
	 *
	 * @return string
	 */
	public function generateInputFieldMultiSelect()
	{
		$selected = array();
		if ($this->fieldValue) {
			$selected = explode(',', $this->fieldValue);
		}

		return $this->form->multiselectarray($this->confKey, $this->fieldOptions, $selected, 0, 0, '', 0, 0, 'style="min-width:100px"');
	}

	/**
	 * Generate input field for select list
	 *
	 * @return string
	 */
	public function generateInputFieldSelect()
	{
		return $this->form->selectarray($this->confKey, $this->fieldOptions, $this->fieldValue, 0, 0, 0, '', 0, 0, 0, '', 'minwidth200');
	}
	...
	public function generateOutputField()
	{
		...
		if ($this->type == 'title') {
			...
			$result = $category->fetch($this->fieldValue);
			if ($result < 0) {
				$this->setErrors($category->errors);
			}
			$ways = $category->print_all_ways(' &gt;&gt; ', 'none', 0, 1); // $ways[0] = "ccc2 >> ccc2a >> ccc2a1" with html formated text
			$toprint = array();
			foreach ($ways as $way) {
				$toprint[] = '<li class="select2-search-choice-dolibarr noborderoncategories"' . ($category->color ? ' style="background: #' . $category->color . ';"' : ' style="background: #bbb"') . '>' . $way . '</li>';
			}
			...
			// Try to print image if it is found
			$image = ($conf->$moduleName->dir_output ?? "") . '/uploads/' . ($this->fieldValue ?? '');
			if (isset($this->fieldValue) && $this->fieldValue != '' && file_exists($image)) {
				$dimensions = dol_getImageSize($image);
				if (!empty($dimensions)) {
				...
				$out .= '<img align="right" height="' . ($dimensions['height'] ?? 'auto') . '" width="' . ($dimensions['width'] ?? "auto") . '" src="' . DOL_URL_ROOT . '/viewimage.php?modulepart=' . $moduleName . '&file=uploads/' . ($this->fieldValue ?? '') . '">';
			} else {
				$out .= '<img align="right" height="40" src="' . DOL_URL_ROOT . '/public/theme/common/nophoto.png">';
			}
			if (!empty($this->fieldValue)) $out .= ' <a href="' . $_SERVER["PHP_SELF"] . '?action=removeImage&const=' . $this->confKey . '&token=' . newToken() . '">' . img_delete($langs->trans("Delete")) . '</a>';
		} else {
			$out .= $this->fieldValue;
		}
	...
	/**
	 * Generate output for multi select fields
	 *
	 * @return string
	 */
	...
	/**
	 * Generate output for color fields
	 *
	 * @return string
	 */
	...
	/**
	 * Generate output for select fields
	 *
	 * @return string
	 */
	...
	{
		$outPut = '';
		if (isset($this->fieldOptions[$this->fieldValue])) {
	 ...
	 * @return self
	 */
	public function setAsImage()
	{
		global $langs;

		$this->type = 'image';
		if (empty($this->helpText)) $this->helpText = $langs->trans("H2G2ImageDefaultHelpText");
		return $this;
	}

	/**
	 * Set type of input as a simple image
	 * no data to store
	 *
	 * @param  string    $type    Type to set
	 * @return self
	 */
	public function setAsOther($type)
	{
		$this->type = $type;
		return $this;
	}
	...
	/**
	 * Set for HTML types, this fields will be large and high
	 *
	 * @param  bool    $fullSize    True to set full size, false otherwise
	 * @return self
	 */
	public function setFullSize($fullSize = true)
	{
		if ($this->type == "html") $this->fullSize = $fullSize;
		return $this;
	}
	...
	{
		return ($this->type == "html" && $this->fullSize);
	}
	...
	{
		if ($this->type == "html") $this->height = $height;
		return $this;
	}

	/**
	 * Set for HTML types, this fields will have a button to save content dynamycally
	 *
	 * @param bool $autoSave True to enable auto save button
	 * @return self
	 */
	public function setAutoSave($autoSave = true)
	{
		if ($this->type == "html") $this->autoSave = $autoSave;
		return $this;
	}
	...
	public function generateSaveButton()
	{
		...
		// Only if item is auto save
		if ($this->autoSave) {
			// Generate save button
			$out .= "<span class='fas fa-save H2G2-button-fast-save' script='float: right;' onclick='saveConstant(";
			// Type
			$out .= '"' . $this->type . '",';
			// Const key
			$out .= '"' . $this->confKey . '",';
			// Const key
			if (!empty($this->usedTable)) $out .= '"' . $this->usedTable . '"';
			$out .= ")' title='" . $langs->trans("H2G2SaveTooltip") . "'>";
			$out .= "</span>";
		}
		return $out;
	}
