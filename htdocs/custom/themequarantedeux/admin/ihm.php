...
$langs->loadLangs(array('admin', 'languages', 'other', 'companies', 'products', 'members', 'projects', 'hrm', 'agenda', 'themequarantedeux@themequarantedeux'));

if (!$user->admin) accessforbidden();
if (empty($conf->themequarantedeux->enabled)) accessforbidden();

$action = GETPOST('action', 'aZ09');
$contextpage = GETPOST('contextpage', 'aZ') ?GETPOST('contextpage', 'aZ') : 'theme42ihm'; // To manage different context of search

$mode = GETPOST('mode', 'aZ09') ? GETPOST('mode', 'aZ09') : 'other'; // 'template', 'dashboard', 'login', 'other', 'theme42'
...
$parameters = array('contextpage' => $contextpage, 'action' => $action, 'mode' => $mode);
...
if ($action == 'update') {
	...
	if ($mode == 'theme42') {
		dolibarr_set_const($db, "TC42_POSITION_MENU", GETPOST('TC42_POSITION_MENU', 'int'), 'chaine', 0, '', $conf->entity);
	}
...
llxHeader('', $langs->trans("Setup"), $wikihelp, '', 0, 0, array('/includes/ace/src/ace.js', '/includes/ace/src/ext-statusbar.js', '/includes/ace/src/ext-language_tools.js',), array());
...
dol_fiche_head($head, 'ihm', '', -1, "themes@themequarantedeux");
...
if ($mode == 'login') {
	...
	print ajax_constantonoff("MAIN_HELPCENTER_DISABLELINK", array(), $conf->entity, 0, 0, 0, 0, 0);
	print '</td>';
	print '</tr>';
	...
	$maxfilesizearray = getMaxFileSizeArrayTheme();
...
if ($mode == 'css') {
	print '<span class="opacitymedium">' . $langs->trans("CSSPreventMessagePage") . '</span><br><br>';
	print '<div class="div-table-responsive-no-min">';
	print '<table summary="edit" class="noborder centpercent editmode tableforfield">';
...
if ($mode == 'theme42') {
	...
	print '<div class="div-table-responsive-no-min">';
	print '<table summary="edit" style="margin: 0px" class="noborder centpercent editmode tableforfield">';
	...
	print '<td>'.$langs->trans("TC42MenuManagement").'</td>';
	print '<td>';
	print $form->selectarray('TC42_POSITION_MENU', $listOfMenuPosition, isset($conf->global->TC42_POSITION_MENU) ? $conf->global->TC42_POSITION_MENU : 0);
	print '</td>';
	print '</tr>';
	print '</table>';
	print '</div>';
...
print '<input style="--btn-color: var(--btn-color-success)" class="button button-save reposition buttonforacesave" type="submit" name="submit" value="' . $langs->trans("Save") . '">';
print '<input style="--btn-color: var(--btn-color-warning)" class="button button-cancel reposition" type="submit" name="cancel" value="' . $langs->trans("Cancel") . '">';
print '</div>';
