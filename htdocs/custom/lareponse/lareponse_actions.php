...
/**
 *	\file			htdocs/custom/lareponse/lareponse_actions.php
...
$permissioneditother = $user->rights->lareponse->article->update_other ?? false;
if (!empty($permissioneditother)) {
	$permissiontoadd = $permissioneditother;
}
...
// Action to delete
if ($action == 'delete' && !empty($permissiontodelete)) {
	if (!($object->id > 0)) {
...
// Action clone object
if ($action == 'clone' && !empty($permissiontoadd)) {
	if (1 == 0 && !GETPOST('clone_content') && !GETPOST('clone_receivers')) {
		...
		if (is_object($result) || $result > 0) {
			...
			$tag = new Tag($db);
			$tagList = array();
			foreach ($tag->getArticleTag($object->id) as $tagListId) {
				$tagList[] = $tagListId['id'];
			}
			$tag->linkArticleTag($newid, $tagList);
