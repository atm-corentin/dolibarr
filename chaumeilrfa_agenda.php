...
/**
 *  \file       chaumeilrfa_agenda.php
 *  \ingroup    clichaumeil
 *  \brief      Tab of events on ChaumeilRfa
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->clichaumeil->multidir_output[!empty($object->entity) ? $object->entity : $conf->entity]."/".$object->id;
}
...
if ($object->id > 0) {
	$title = $langs->trans("ChaumeilRfa")." - ".$langs->trans('Agenda');
	//$title = $object->ref." - ".$langs->trans("Agenda");
	$help_url = 'EN:Module_Agenda_En|DE:Modul_Terminplanung';

	llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-clichaumeil page-card_agenda');
	...
	print dol_get_fiche_head($head, 'agenda', $langs->trans("ChaumeilRfa"), -1, $object->picto);
	...
	if (isModEnabled('agenda') && ($user->hasRight('agenda', 'myactions', 'read') || $user->hasRight('agenda', 'allactions', 'read'))) {
		...
		//$cachekey = 'count_events_chaumeilrfa_'.$object->id;
